...
package org.flowplayer.view {
	...
	internal class VideoDisplay extends AbstractSprite implements MediaDisplay {
		...
		private var _overlayMask:Sprite = null;
		...
		private function createOverlay():void {
			...
			_overlay = new Sprite();
			addChild(_overlay);
			_overlay.graphics.beginFill(255, 0.0);
			_overlay.graphics.drawRect(0, 0, 10, 10);
			_overlay.graphics.endFill();
		}
		private function createOverlayMask() : void {	
			_overlayMask = new Sprite();
			addChild(_overlayMask);
			_overlayMask.graphics.beginFill(_clip.maskc, _clip.maska);
			_overlayMask.graphics.drawRect(0, 0, 10, 10);
		...
		override protected function onResize():void {
			...
			if (null != _overlayMask)
			{
				_overlayMask.x = this.width * _clip.maskx; 
				_overlayMask.y = this.height * _clip.masky; 
				_overlayMask.width = this.width * _clip.maskw; 
				_overlayMask.height = this.height * _clip.maskh; 
			}
		}
		...
		public function init(clip:Clip):void {
			...
			if (null != _overlayMask)
			{
				swapChildren(_overlayMask, video);
			}
		}
