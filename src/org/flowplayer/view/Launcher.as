...
//            lookupSlowMotionPlugin(_flowplayer);
		...
		{
			if (_config.getMaskConfiguration().url.length <= 0)
				return;
			...
			ldr.mask = rect; 
			var url:String = _config.getMaskConfiguration().url; 
...
//			_flowplayer.onFullscreenExit(onFullscreen);
		...
		private function createFlashVarsConfig():void {
            ...
            var configStr:String = 
				//Preloader(root).injectedConfig || 
				root.loaderInfo.parameters["config"];
...
//			addViewLiteners(_screen);
		...
		private function createLogoForCanvas():void {
			...
			_canvasLogo = new BuiltInConfig.CanvasLogo();
