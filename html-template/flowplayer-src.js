...
// {{{ public methods
	...
	extend(self, {
		...
		isLoaded: function() 
		{
			log(["isLoaded", api, null != api ? api.fp_play : "null", isUnloading]);
		...
		load: function(fn) {
			if (!self.isLoaded() && self._fireEvent("onBeforeLoad") !== false) {
				var onPlayersUnloaded = function() {
					...
					log(["flashembed", wrapper, params, {config: conf}]);
      ...
      getPlugin: function(name) {
		...
		_api: function() {
			if (!self.isLoaded()) {
				throw "ohNOO " +self.id()+ " not loaded when calling an API method";
			}
			return api;
		},
...
// {{{ init
   ...
   function init() {
		...
		log(["playerId,apiId", playerId, apiId]);
	...
	if (typeof wrapper == 'string') {
		var node = el(wrapper);
		if (!node) { throw "ohNoo we cannot access element: " + wrapper; }
		wrapper = node;
		init();
...
window.flowplayer = window.$f = window.$link = function() {
	log("player instance");
